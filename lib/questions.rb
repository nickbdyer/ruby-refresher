...
def separate_array_into_even_and_odd_numbers(array)
  [ array.select {|elem| elem.even?} , array.select {|elem| elem.odd?} ]
end
...
def get_all_letters_in_array_of_words(array)
  ...
  array.flatten.sort
  # array.length.times{ array.shift.each_char { |c| array << c } }
  # array.sort
end
...
def titleize_a_string(string)
  string = string.split.each do |word| 
    word.capitalize! unless ["the", "and", "a"].include?(word)
  end
  string[0].capitalize!
  string.join(" ")
end
...
def is_a_2014_bank_holiday?(date)
  dates = ["01/01", "18/04", "21/04", "05/05", "26/05", "25/08", "25/12", "26/12"]
  dates.include?(date.strftime("%d/%m"))
end
